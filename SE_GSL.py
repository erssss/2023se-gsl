...
def encode_community(edge_index, logits, node_num, device, se):
    ...
    adj_matrix = reweight(node_num, edge_index_2, logits)

    encode_tree = PartitionTree(adj_matrix=np.array(adj_matrix))
...
def CombineDelta(node1, node2, cut_v, g_vol):
    ...
    v12 = v1 + v2
    return (
        (v1 - g1) * math.log2(v12 / v1)
        + (v2 - g2) * math.log2(v12 / v2)
        - 2 * cut_v * math.log2(g_vol / v12)
    ) / g_vol
    ...
    def build_sub_leaves(self, node_list, p_vol):
        ...
        for vertex in node_list:
            ori_ent += -(self.tree_node[vertex].g / self.VOL) * math.log2(
                (self.tree_node[vertex].vol + 1) / (p_vol + 1)
            )
            sub_n = set()
            vol = 0
...
def get_community(encode_tree: PartitionTree):
    node_dict = encode_tree.tree_node
    root_id = encode_tree.root_id
...
def get_sedict(community: list, encode_tree: PartitionTree):
    node_dict = encode_tree.tree_node
    se_dict = {}
    for community_id in community:
        ...
        for i, e in enumerate(node_list):
            e = node_dict[e]
            e: PartitionTreeNode
            se[i] = -(e.g / encode_tree.VOL) * torch.log2(
                torch.tensor((e.vol + 1) / (node_dict[e.parent].vol + 1))
            ) + encode_tree.deduct_se(community_id, None)
            # se[i] = -(e.g / encode_tree.VOL) * torch.log2(
            #     torch.tensor((e.vol + 1) / (node_dict[e.parent].vol + 1)))
...
def select_link(
    community_id: int, encode_tree: PartitionTree, isleaf: torch.Tensor, se_dict
):
    node_dict = encode_tree.tree_node
    node_list = list(node_dict[community_id].children)
    node_dict = encode_tree.tree_node
...
def select_leaf(node_id, encode_tree: PartitionTree, isleaf: torch.Tensor, se_dict):
    # print(node_id)
    node_dict = encode_tree.tree_node
...
def reshape(community: list, encode_tree: PartitionTree, isleaf: torch.Tensor, k):
    se_dict = {}
    edge_index = []
    node_dict = encode_tree.tree_node
    se_dict = get_sedict(community, encode_tree)
    ...
    for community_id in community:
        ...
        for i in range(prefer_edge_num):
            id1, id2 = select_link(community_id, encode_tree, isleaf, se_dict)
            edge_index.append(
                [
                    select_leaf(id1, encode_tree, isleaf, se_dict),
                    select_leaf(id2, encode_tree, isleaf, se_dict),
                ]
            )
    edge_index = torch.tensor(edge_index)
    edge_index = torch.concat((edge_index, torch.flip(edge_index, dims=[1])), dim=0)
